...
var citySelect = function(){
  ...
  function init(one,two,tree){
    ...
    one.on('change', function(event) {
      selVal = $(this).val();
      if(selVal !== 0){
        tree.html("<option value='0'>--请选择--</option>");
        two.html("<option value='0'>--请选择--</option>");
        for (twoCity in citysObj[selVal]) {
          domOpt = "<option value='"+twoCity+"'>"+twoCity+"</option>";
          two.append(domOpt);
        }
      }
    });
    ...
    two.on('change', function(event) {
      selVal = $(this).val();
      oneVal = one.val();
      treeArr = citysObj[oneVal][selVal];
      if(selVal !== 0){
        tree.html("<option value='0'>--请选择--</option>");
        for(var i=0;i<treeArr.length;i++){
          domOpt = "<option value='"+treeArr[i]+"'>"+treeArr[i]+"</option>";
          tree.append(domOpt);
        }
      }
    });
